...
export function ObservacionesModal({ consultationId, onCerrar, onCreado }: Props) {
  ...
  async function generar() {
    if (!texto.trim()) {
      setError('Escribí las observaciones antes de generar el documento.');
      return;
    }
    setEnviando(true);
    setError(null);
    try {
      const res = await fetch(`/api/consultations/${consultationId}/observaciones`, {
        method: 'POST',
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="observaciones-titulo"
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <h2 id="observaciones-titulo" className="text-2xl font-bold text-[#2C3E50]">
            Crear observaciones
          </h2>
          <button
            onClick={onCerrar}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Escribí acá las observaciones de la consulta..."
            autoFocus
          />
        </div>
